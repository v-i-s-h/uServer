...
GetTicks(void)
{
    ...
    // Return the number of ticks since the last time this was called.
    return(ui32RetVal);
}
...
//*****************************************************************************
void
ReadLine( char *cmdBuf, uint16_t buffLen )
{
    tState eStateCopy;
    uint32_t ui32DriveTimeout, ui32Prompt;
    ...
    while(1) {
        ...
        if(ui32Prompt) {
            ...
            if(g_eState == STATE_DEVICE_READY) {
                UARTprintf("%s> %s", g_cCwdBuf, cmdBuf);
            }
            else if(g_eState == STATE_UNKNOWN_DEVICE) {
                UARTprintf("UNKNOWN> %s", cmdBuf);
            } else {
                UARTprintf("NODEV> %s", cmdBuf);
            }
